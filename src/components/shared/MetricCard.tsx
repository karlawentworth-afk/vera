...
export function MetricCard({ label, value, unit, trend, color, href }: MetricCardProps) {
  const content = (
    <>
  ...
  if (href) {
    return (
      <Link to={href} className="bg-white border border-gray-200 rounded-lg p-4 relative overflow-hidden hover:border-gray-300 hover:shadow-sm transition cursor-pointer block">
        {content}
      </Link>
    )
  }
